...
- **`forget` only deletes entries that contain the query** (#8, #23). It used to
  ...
  exactly the entries citing those ids. If several files carry the id, nothing is
  deleted and the message names them — an id lookup used to take whichever file a
  directory listing returned first. `forget` deletes exactly the file it checked;
  ...
  after editing files by hand run `rebuild-index`. `forget` works on the project
  store only. The CLI rejects a `--scope` other than `entry`/`topic` and a
