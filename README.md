...
| `forget` | `{ query, scope, confirm }` | Works on the project store only. A `query` that is one entry id deletes exactly that entry, or nothing if no entry has it; if several files have that id, nothing is deleted and the message names them. An id is a registered prefix and a number (`dec-012`, `note 130`, `note #130`, `DEC‑012`, `note #١٣٠`; the prefixes are `dec`, `entity`, `inc`, `pat`, `wf`, `note`, `session` and those of installed extensions) or a UUID. Case, any spaces or punctuation between the parts, digits of any script and surrounding punctuation are ignored. A prefix and a number always count as an id (`session 3`), even if an entry contains those words as text; `gpt-5`, with no registered prefix, is text. A query that contains ids in a list or a sentence is refused. Any other `query` matches entries whose title or text contains it as a phrase: its words as whole words, in order, only spaces or punctuation between them, case-insensitive (`rat` does not match `rater`; a stem-only match is not enough). `forget` never picks from the matches: `scope: "entry"` deletes the one matching entry, `"topic"` up to 10. With more matches nothing is deleted, and the message gives the count and up to 10 ids (`success: false`). A query that matches nothing deletes nothing. A query full-text search cannot run (`A`, `OR`) deletes nothing and says so. The phrase is checked against the file itself, but the candidates come from the search index: after editing files by hand, run `rebuild-index`, or text the index does not know yet is not found. If a delete fails half-way, forget stops and the answer lists the files already gone (`Forgot 1 of 2, then could not delete …`) |
