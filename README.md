...
| `forget` | `{ query, scope, confirm }` | A `query` that is one entry id (`dec-012`, `note 130`, `DEC‑012`, `note #130`, a UUID; case, any spaces or punctuation between prefix and number, and surrounding punctuation ignored) deletes exactly that entry, or nothing if no entry has it. An id prefix followed by a number always counts as an id (`session 3`, `note 5`; the prefixes are `dec`, `entity`, `inc`, `pat`, `wf`, `note`, `session` and those of installed extensions), even if an entry contains those words as text. A query that contains ids in a list or a sentence is refused. Any other `query` deletes entries whose title or text contains it as a phrase: its words as whole words, in order, only spaces or punctuation between them, case-insensitive (`rat` does not match `rater`; a stem-only match is not enough). `scope: "entry"` deletes the best match, `"topic"` up to 10. A query that matches nothing deletes nothing |
