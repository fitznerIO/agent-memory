...
| `forget` | `{ query, scope, confirm }` | An id-shaped `query` (`dec-012`, a UUID; case and surrounding punctuation ignored) deletes exactly that entry, or nothing if no entry has it. Any other `query` deletes entries whose title or text contains it as a phrase: its words in order, only spaces or punctuation between them, case-insensitive (`soup` matches `soups`; a stem-only match is not enough). `scope: "entry"` deletes the best match, `"topic"` up to 10. A query that matches nothing deletes nothing |
